  ...
  creditReports: router({
    ...
    savePreviewAnalysis: optionalAuthProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Expand any preview with bureau "TransUnion, Equifax" into one per bureau (real data, no placeholders)
        const expandedPreviews: Array<{ name?: string; last4?: string; status?: string; balance?: string; amountType?: string; bureau?: 'transunion' | 'equifax' | 'experian' }> = [];
        ...
        for (const p of rawPreviews) {
          ...
          const bureausForPreview = parts.some((b) => bureauList.includes(b as any))
            ? parts.filter((b): b is typeof bureauList[number] => bureauList.includes(b as any))
            : [('transunion' as const)];
          if (bureausForPreview.length === 0) bureausForPreview.push('transunion');
          for (const b of bureausForPreview) {
            expandedPreviews.push({ ...p, bureau: b });
          }
        }
        const previewsToSave = expandedPreviews.length > 0 ? expandedPreviews : rawPreviews;
        ...
        // Save account previews as negative accounts — real data only, no placeholders
        if (previewsToSave.length > 0) {
          const previews = previewsToSave as Array<{ name?: string; last4?: string; status?: string; balance?: string; amountType?: string; bureau?: 'transunion' | 'equifax' | 'experian' }>;
          console.log('[SavePreview] Saving', previews.length, 'accounts (real data, 1 per account×bureau)');
          for (const preview of previews) {
            ...
            const bureau = preview.bureau || 'transunion';
  ...
  payments: router({
    ...
    createSubscription: optionalAuthProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!priceId) {
          console.error('[Checkout] Price ID not configured for tier:', input.tier);
        ...
        try {
          ...
          try {
            ...
            console.error('[Checkout] Subscription creation error:', subError.message);
            console.error('[Checkout] Stripe error details:', JSON.stringify(subError, null, 2));
            throw new Error(`Failed to create subscription: ${subError.message}`);
          }
          
          console.log('[Checkout] Latest invoice type:', typeof subscription.latest_invoice);
          console.log('[Checkout] Latest invoice value:', subscription.latest_invoice);
          
          // Handle latest_invoice - it might be a string ID or an expanded Invoice object
          let invoice: Stripe.Invoice;
          if (!subscription.latest_invoice) {
            console.error('[Checkout] Subscription has no latest_invoice field');
            throw new Error('Subscription created but no invoice was generated. Please try again.');
          } else if (typeof subscription.latest_invoice === 'string') {
            // It's a string ID, retrieve it
            console.log('[Checkout] Retrieving invoice by ID:', subscription.latest_invoice);
            try {
              ...
              console.error('[Checkout] Invoice retrieval error:', invoiceError.message);
              throw new Error(`Failed to retrieve invoice: ${invoiceError.message}`);
            }
            ...
            console.error('[Checkout] Unexpected invoice type:', typeof subscription.latest_invoice);
            throw new Error('Failed to process subscription invoice - unexpected format');
          }
