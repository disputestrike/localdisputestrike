...
6. The letter is generated by "DisputeStrike" (NOT DisputeStrike or any other name)
...
7. **PROFESSIONAL FORMATTING:**
   - Consumer's ACTUAL name and address (from provided data) at top
   - Bureau's address
   - Today's date (formatted as "Month Day, Year" e.g. "January 8, 2026")
   - ONE RE: line only
   - Body with legal citations
   - ONE signature block at the very end only
...
**IMPORTANT: Use these EXACT values - DO NOT use placeholder text like [Your Name] or [Date]:**
...
1. Starts with the consumer's name and address (use "${userName}" and "${currentAddress}" exactly)
2. Uses today's date: ${formattedDate}
3. Has ONE RE: line citing FCRA § 1681i
4. Analyzes each account with specific violations
5. Demands deletion of unverifiable items
6. Has ONE signature block at the very end with "${userName}"
7. NO placeholder text - all fields must be filled with actual data

Remember: This letter is generated by DisputeStrike.`;
}
