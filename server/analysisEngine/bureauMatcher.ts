...
function toNormalized(account: Record<string, unknown>, bureau: string) {
  ...
  const originalCreditor = String(account.originalCreditor || '').trim();
  let balance = 0;
  ...
  return { creditorName: name, accountNumber: last4, balance, dateOpened, originalCreditor };
}
...
export function isSameAccount(
  ...
  // Different account numbers = different accounts (e.g. two cards from same bank)
  if (n1.accountNumber && n2.accountNumber && n1.accountNumber !== n2.accountNumber) return false;
  ...
  // When BOTH lack account numbers: different originalCreditors = different accounts (e.g. 2 LVNV collections)
  if (!n1.accountNumber && !n2.accountNumber && n1.originalCreditor && n2.originalCreditor) {
    const origScore = fuzzyMatch(normalizeCreditorName(n1.originalCreditor), normalizeCreditorName(n2.originalCreditor));
    if (origScore < 70) return false;
  }
  return true;
}
...
export function matchAccountsAcrossBureaus(
  ...
  const makeKey = (a: Record<string, unknown>, bureau: string, idx: number) => {
    const name = String(a.accountName || a.creditor_name || '').toLowerCase();
    const num = String(a.accountNumber || a.account_number || '');
    const last4 = num.replace(/\D/g, '').slice(-4);
    const bal = typeof a.balance === 'number' ? a.balance : parseFloat(String(a.balance || '0').replace(/[$,]/g, '')) || 0;
    const orig = String((a.originalCreditor as string) || '').toLowerCase().slice(0, 15);
    // Same creditor + empty last4 = multiple accounts. Use balance+originalCreditor+idx to guarantee uniqueness.
    const suffix = last4 || `${bal}-${orig}-${idx}`;
    return `${bureau}:${name}:${suffix}`;
  };

  const all = [...tu, ...eq, ...ex];
  const accountToIdx = new Map<Record<string, unknown>, number>();
  all.forEach((a, i) => accountToIdx.set(a, i));

  for (let idx = 0; idx < all.length; idx++) {
    const account = all[idx];
    const bureau = getBureauCode(account);
    const k = makeKey(account, bureau, idx);
    ...
    for (const { list, key: bk } of lists) {
      ...
      for (const other of list) {
        ...
        if (ob !== bk) continue;
        const otherIdx = accountToIdx.get(other) ?? -1;
        const ok = makeKey(other, ob, otherIdx);
